...
import { useState, useEffect, useRef } from 'react';
import axios from 'axios';
import { AlertCircle, X, Activity, Mic } from 'lucide-react'; // Added Mic icon
import {
  StreamVideo,
  StreamVideoClient,
  StreamCall,
  ...
  useEffect(() => {
    ...
    const setupStream = async () => {
      ...
      try {
        ...
        // This ensures the Vision Agent can see AND hear the patient
        await myCall.camera.enable();
        await myCall.microphone.enable(); // CRITICAL: This allows Deepgram to hear the user
    ...
    return () => {
      ...
      if (myCall) {
        myCall.camera.disable().catch(console.error);
        myCall.microphone.disable().catch(console.error); // Cleanup mic
        myCall.leave().catch(console.error);
      }
      if (myClient) {
        myClient.disconnectUser().catch(console.error);
